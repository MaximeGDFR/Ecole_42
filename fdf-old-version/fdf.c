...
/*   Updated: 2024/12/10 15:57:31 by maximegdfr       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	init_bitmap(&data, WIDTH, HEIGHT);
	if (!data.bitmap.img)
		handle_error("Error: create bitmap failed.\n", 1);
	mlx_put_image_to_window(data.mlx, data.win, data.bitmap.img, WIDTH - MENU_WIDTH, 0);
...
void	start_program(t_data *data, char *filename)
{
	...
	if (!data->map)
		handle_error("Error loading map.\n", 1);
